...
def TC_FlipkartCollection(iterations=1, vuser=1, ramp=1):
	...
	if(int(vuser) == 1): 
		...
		cmd = "newman run FlipkartCollection.json -r html --reporter-html-export result.html -x -n "+str(iterations)
		os.system(cmd)
	elif(int(vuser) > 1):
		for i in vuser:
			os.chdir("D:/TON/WebTesting/API/GUI/Demo_TON/FlipkartCollection")
			cmd = "newman run FlipkartCollection.json -r html --reporter-html-export result_"+str(i)+".html -x -n "+str(iterations)
			os.system(cmd)
