...
class viewAnalytics:
	...
	def setIndex(self,baseFolder):
		...
		command = "elasticdump --input="+baseFolder+"\WebTesting\Browser\GUI\DEMO/createIndex.json --output=http://localhost:9200/.kibana"
		os.system(command)

	def setVisualization(self,baseFolder):

		command = "elasticdump --input="+baseFolder+"\WebTesting\Browser\GUI\DEMO/hai.json --output=http://localhost:9200/.kibana"
		os.system(command)
